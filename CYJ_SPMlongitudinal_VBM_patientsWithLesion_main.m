function CYJ_SPMlongitudinal_VBM_patientsWithLesion(T1s,T2s,subjIDs,lesionmasks,timeNumber,TPM,DATREL_templates_1_6,masks_tobeBringBack,subLesionMNI_flag,segment_flag)
...
T1s=T1s(ind);subjIDs=subjIDs(ind);lesionmasks=lesionmasks(ind);timeNumber=timeNumber(ind);

%% T1 sublesion cost function
...
fun_subLesion=@(x,y,z,l) CYJ_Image_assignMask_WithValue(x,y,0,l,[z '_subLesion'],0);
cellfun(fun_subLesion,T1s,lesionmasks,T1names,outpaths);
T1s_subLesion=cellfun(@(x,y) [x '/' y '_subLesion.nii'],outpaths,T1names,'UniformOutput',false);

%% longitudinal_registration
CYJ_SPMlongitudinal_registration(T1s_subLesion,timeNumber);
%% bring lesionmask to avg
deformation2Avgs=cellfun(@(x,y) [x '/y_' y '_subLesion.nii'],outpaths,T1names,'UniformOutput',false);
avgT1=[outpaths{1} '/avg_' T1names{1} '_subLesion.nii'];
CYJ_SPM_native2MNI_using_DeformationNative2MNI(lesionmasks{1},deformation2Avgs{1},avgT1,outpaths{1},'lesionMInAvg',7,0);
lesionMInAvgs=[outpaths{1} '/lesionMInAvg_' lesionMnames{1} '.nii'];
CYJ_SPM_thr_Image(lesionMInAvgs,0.2,outpaths{1},['lesionMInAvg_' lesionMnames{1}]);
% T1 avg sub lesionmask
CYJ_Image_assignMask_WithValue(avgT1,lesionMInAvgs,0,outpaths{1},['avgT1_forSegment_' subjIDs{1}],0);
...
if ~isempty(T2s)
    ...
    avgT2=[outpaths{1} '/avgT2_avg_' T1names{1} '_subLesion.nii'];
    % T2 avg sub lesionmask
    CYJ_Image_assignMask_WithValue(avgT2,lesionMInAvgs,0,outpaths{1},['avgT2_forSegment_' subjIDs{1}],0);
    % do segment
    if strcmp(segment_flag,'c')
        CYJ_VBM_segment_T1T2([outpaths{1} '/avgT1_forSegment_' subjIDs{1} '.nii'],[outpaths{1} '/avgT2_forSegment_' subjIDs{1} '.nii'],TPM);
    elseif strcmp(segment_flag,'p')
    end
else
    % do segment
    if strcmp(segment_flag,'c')
        CYJ_VBM_segment_T1T2([outpaths{1} '/avgT1_forSegment_' subjIDs{1} '.nii'],'',TPM);
    elseif strcmp(segment_flag,'p')
        CYJ_VBM_segment_T1_CAT12_7([outpaths{1} '/avgT1_forSegment_' subjIDs{1} '.nii'],TPM,1.5,'eastern','dartel',DATREL_templates_1_6{1});
movefile([outpaths{1} '/mri/p1avgT1_forSegment_' subjIDs{1} '.nii'],outpaths{1});
movefile([outpaths{1} '/mri/p2avgT1_forSegment_' subjIDs{1} '.nii'],outpaths{1});
movefile([outpaths{1} '/mri/p3avgT1_forSegment_' subjIDs{1} '.nii'],outpaths{1});
movefile([outpaths{1} '/mri/rp1avgT1_forSegment_' subjIDs{1} '_rigid.nii'],outpaths{1});
movefile([outpaths{1} '/mri/rp2avgT1_forSegment_' subjIDs{1} '_rigid.nii'],outpaths{1});
    end
end
%% do DARTEL
forDARTEL1=cell2mat(g_ls([outpaths{1} '/r' segment_flag '1avgT1_forSegment_' subjIDs{1} '*.nii']));
forDARTEL2=cell2mat(g_ls([outpaths{1} '/r' segment_flag '2avgT1_forSegment_' subjIDs{1} '*.nii']));
CYJ_dartel_existingTemplate(forDARTEL1,forDARTEL2,DATREL_templates_1_6);

%% c1 c2 c3 * jd
gray=[outpaths{1} '/' segment_flag '1avgT1_forSegment_' subjIDs{1} '.nii'];
white=[outpaths{1} '/' segment_flag '2avgT1_forSegment_' subjIDs{1} '.nii'];
csf=[outpaths{1} '/' segment_flag '3avgT1_forSegment_' subjIDs{1} '.nii'];
jd2Avgs=cellfun(@(x,y) [x '/j_' y '_subLesion.nii'],outpaths,T1names,'UniformOutput',false);
...
%% normalise_to_MNI
flowfield=cell2mat(g_ls([outpaths{1} '/u_r' segment_flag '1avgT1_forSegment_' subjIDs{1} '*.nii']));
...
if subLesionMNI_flag==1
    MNI_grayXjds_name=cellfun(@(y) ['mwgrayXjd_' y],subjIDs,'UniformOutput',false);
    MNI_whiteXjds_name=cellfun(@(y) ['mwwhiteXjd_' y],subjIDs,'UniformOutput',false);
    MNI_csfXjds_name=cellfun(@(y) ['mwcsfXjd_' y],subjIDs,'UniformOutput',false);
    fun=@(x,y,z,l) CYJ_Image_assignMask_WithValue(x,y,0,z,l,0);
    cellfun(fun,[MNI_grayXjds;MNI_whiteXjds;MNI_csfXjds],[lesionMInMNI;lesionMInMNI;lesionMInMNI],[outpaths;outpaths;outpaths],[MNI_grayXjds_name;MNI_whiteXjds_name;MNI_csfXjds_name]);
end
%% split hemi
